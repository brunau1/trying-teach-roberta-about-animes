...
def main(model, version):
    ...
    for search_text in search_phrases:
        ...
        results = find_similar(model, search_text, sinopses_embeddings, k=10)
    ...
    out_path = os.path.abspath(os.path.join(
        os.path.dirname(__file__), '..', 'public', f"STS_roberta_15k_{version}.txt"))
...
for model_number in model_numbers:
    roberta_trained_model_path = os.path.abspath(os.path.join(
        os.path.dirname(__file__), '..', 'public', 'checkpoint', 'roberta_trained_model_checkpoint', model_number))

    sts_model = SentenceTransformer(roberta_trained_model_path)

    sts_model.to(DEVICE)

    main(sts_model, model_number)
